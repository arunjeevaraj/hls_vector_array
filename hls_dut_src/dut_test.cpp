...
int writebinary(const char* file_name, uint32_t size, void* data_ptr);
...
int writebinary(const char* file_name, uint32_t size, void* data_ptr)
{
	...
	fwrite(data_ptr, sizeof(char), size, fp);
	fclose(fp);
	return 0; // sucess;
}
		...
		uint64_t *mismatches, double *mse)
{
	...
	*mismatches = 0;
	double sum = 0;
	for (unsigned j = 0; j < FEATURE_SET*PARTICLE_CNT; j++) {
	...
	*mse = sqrt(sum/(FEATURE_SET*PARTICLE_CNT));
}
...
int main()
{
	...
	printf("----------------------\n");
	printf("Writing binary file for microblaze test.\n");
	writebinary("frame_in.bin",
		sizeof(dut_frame_data_t)*PARTICLE_CNT*FEATURE_SET, (void*)frame_i_dut);
	writebinary("frame_out.bin",
		sizeof(dut_frame_data_t)*PARTICLE_CNT*FEATURE_SET, (void*)frame_o_dut);
	writebinary("trans_mat.bin",
		sizeof(dut_tmat_data_t)*FEATURE_SET*FEATURE_SET, (void*)tm_dut);
	printf("Writing binary files done.\n");
	printf("----------------------\n");
